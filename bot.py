...
def cluster_items(items, min_shared=3):
    ...
    for i in range(len(items)):
        if used[i]:
            continue
        seed_ts, seed_title, seed_source, seed_link = items[i][0], items[i][1], items[i][2], items[i][3]
...
def render_column(items):
    ...
    for cluster in clusters:
        if len(cluster) == 1:
            ts, title, source, link = cluster[0][0], cluster[0][1], cluster[0][2], cluster[0][3]
            ...
            for _item in cluster:
                ts, title, source, link = _item[0], _item[1], _item[2], _item[3]
                friendly = get_friendly_source(source)
                ts_str = ts_to_pdt(ts)
...
def build_trending_topics(max_topics=10):
    ...
    for _it in all_items:
        ts, title, source, link = _it[0], _it[1], _it[2], _it[3]
...
try:
    ...
    for _ri in rss_items:
        ts, title, source, link = _ri[0], _ri[1], _ri[2], _ri[3]
...
try:
    ...
    for section_name, items in sections:
        for _fi in items:
            ts, title, source, link = _fi[0], _fi[1], _fi[2], _fi[3]
            feed_items.append({
